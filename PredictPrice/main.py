...
def Processing(data):
    ...
    for x in range(len(data['ts']) - 1):
        ...
        count = delta.days * 8
        # count = delta.days
        new_data.append( data['prices'][x])
        while count != 1 and count > 0:
            if data['prices'][x] <= data['prices'][x + 1]:
                temp = random.randint(int(data['prices'][x]), int(data['prices'][x+1]))

            if data['prices'][x] > data['prices'][x + 1]:
                temp = random.randint(int(data['prices'][x+1]), int(data['prices'][x]))
    ...
    dataset_test = data['prices'][data_end:]
    ...
    while i<720:
        ...
        if i % 7 == 0:
            ...
            dt = last_date_time.strftime('%d/%m/%Y')
            results['prices'].append(predicted_stock_price[0][0])
